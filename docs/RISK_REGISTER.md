...
| R2 | **Rate limiting is IP-only** without auth — bypassable via IP rotation | M×M | Token-bucket per IP on writes → 429; **also per-instance/in-memory** (effective limit is N× across N instances, resets on restart). First line only; real fix = API keys + shared Redis store (out of chosen scope). | Accepted |
