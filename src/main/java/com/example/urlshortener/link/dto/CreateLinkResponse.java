...
/**
 * Response returned once, at creation time. Includes the {@code managementToken} — the capability
 * required to later delete the link. It is shown exactly once and never retrievable again (only its
 * hash is stored), so clients must persist it if they need it.
 */
